...
def main():
    ...
    # 1ページ分繰り返し
        print(f"会社名が{len(name_list)}件見つかりました")
        print(f"タイトルが{len(copy_list)}件見つかりました")
        for name in name_list:
            exp_name_list.append(name.text)
            print(f"{count_name}件目" + name.text)
            count_name +=1

        for copy in copy_list:
            exp_copy_list.append(copy.text)
            print(f"{count_copy}件目" + copy.text)
            count_copy +=1
    ...
    df = pd.DataFrame({"会社名":exp_name_list,"タイトル":exp_copy_list})
    df.to_csv("data.csv", encoding = "cp932")
